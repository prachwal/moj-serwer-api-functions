    ...
    for (const channel of channels) {
      try {
        ...
        const existingVideos = await videosCollection
          .find({ video_id: { $in: videoItems.map(v => v.videoId) } })
          .toArray();
        
        // Create map for quick lookup
        existingVideos.forEach(video => {
          existingVideosMap.set(video.video_id, video);
        });
        
        logger.info(`Processing ${videoItems.length} videos for channel ${channel.channel_name} (${existingVideos.length} already in database)`);
        
        // Process each video
        for (const item of videoItems) {
          try {
            // Get full video details
            const videoDetails = await getVideoDetails(item.videoId, logger);
            
            // Check if video already exists in database
            const existingVideo = existingVideosMap.get(item.videoId);
            const isNew = !existingVideo;
            
            // Create video data object, preserving sentiment if video exists
            const videoData = {
            ...
            if (isNew) {
              ...
              if (textToAnalyze) {
                sentiment = await analyzeSentiment(textToAnalyze, process.env, logger);
            ...
            try {
              ...
              const existingSubtitles = await subtitlesCollection.findOne({ video_id: item.videoId });
              
              if (existingSubtitles && existingSubtitles.subtitles && existingSubtitles.subtitles.length > 0) {
                logger.debug(`Subtitles already exist for video: ${item.videoId}`);
                videoData.has_subtitles = true;
              } else {
                logger.info(`Fetching subtitles for video: ${item.videoId}`);
                await fetchAndStoreSubtitlesForVideo(item.videoId, mongo, logger);
                videoData.has_subtitles = true;
              }
            } catch (error) {
              logger.warn(`Failed to fetch subtitles for video ${item.videoId}: ${error.message}`);
            ...
            if (isNew) {
              ...
              logger.info(`Added new video: ${videoData.title} (${item.videoId})`);
            } else {
              await videosCollection.updateOne(
                { video_id: item.videoId },
                { $set: videoData }
              );
              logger.info(`Updated existing video: ${videoData.title} (${item.videoId})`);
            }
